function attachLoadMoreListener() {
    document.getElementById('load-more-button').addEventListener('click', async function () {
        ...
        // Get the date from the last GOTW card (fix this in v1.3.0, this is a horrible system)
        const gotwCards = document.querySelectorAll('.gotw-card');
        if (wotdCards.length === 0) {
            ...
            const lastCard = gotwCards[gotwCards.length - 1];
        ...
        try {
            ...
            let cardsHTML = '';
            results.forEach(gotw => {
            // Use the date directly from the API
            const dateParts = gotw.date.split('-');
