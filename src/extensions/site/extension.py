...
def get_gotw():
    ...
    ipa = gotw.ipa
    pos = gotw.pos
    definition = gotw.definition
    date = gotw.date
    current_date = gotw.current_date
...
def get_previous_wotd():
    ...
    global has_more
    query_result = gotw.query_previous(date=gotw.current_date, limit=3)
...
def set_inject_html():
    global inject_html, word, ipa, pos, definition, date_formatted, previous_wotds
    # HTML for Grammar of the Week
    inject_start_html = '''
        <main class="site-main container">
    '''
    if word:
        week_number = datetime.datetime.strptime(current_date, '%d-%m-%Y').isocalendar()[1]
        wotd_current_html = f'''
            <h1 class="heading">Grammar of the Week {week_number}:</h1>
        ...
        wotd_current_html = '''
            <h1 class="heading">Grammar of the Week:</h1>
            <section class="no-wotd" style="text-align: center; margin: 1rem;">
                <p>No Grammar of the Week available at this time :(</p>
            </section>
        '''

    if previous_wotds:
        wotd_previous_html = '<div class="divider-horizontal"></div><h1 class="heading">Previous Grammar of the Week entries:</h1>'
        for wotd in previous_wotds:
            ...
            previous_week_number = datetime.datetime.strptime(wotd['date'], '%d-%m-%Y').isocalendar()[1]
            wotd_previous_html += f'''
                <section class="card wotd-card">
                    ...
                    <p id="date" class="date">Week {previous_week_number} - {previous_date_formatted}</p>
                </section>
            '''
    else:
        wotd_previous_html = ''
...
def www_query_previous():
    date = request.args.get('date', default=gotw.current_date)
    limit = 3
    try:
        previous_wotds = gotw.query_previous(date, limit=limit)
        if previous_wotds:
...
def api_query():
    date = request.args.get('date', default=gotw.current_date)
    try:
        word_data = gotw.query_word(date=date)
        if word_data:
