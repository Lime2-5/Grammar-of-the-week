...
embed_templates = {
    'subscribe': {
        'private': {
            'title': '**Subscribed** to Grammar of the Week!',
            'description': '''You will now receive the Grammar of the Week each week at <t:0:t>.
        ...
        'guild': {
            'title': '**Subscribed** to Grammar of the Week!',
            'description': '''This channel will now receive the Grammar of the Week each week at <t:0:t>.
            To configure settings, send `/config`.
            To unsubscribe, send `/unsubscribe`.''',
            'color': discord.Color.gold()
        },
        'already': {
            'private': {
                ...
                'description': '''You are already subscribed to the Grammar of the Week.
                To configure settings, send `/config`.
                To unsubscribe, send `/unsubscribe`.''',
            ...
            'guild': {
                ...
                'description': '''This channel is already subscribed to the Grammar of the Week.
                To configure settings, send `/config`.
                To unsubscribe, send `/unsubscribe`.''',
                'color': discord.Color.pink()
            }
    ...
    'unsubscribe': {
        'private': {
            'title': '**Unsubscribed** from Grammar of the Week.',
            'description': '''You will no longer receive the Grammar of the Week.
            To subscribe again, send `/subscribe`.''',
            'color': discord.Color.blue()
        },
        'guild': {
            'title': '**Unsubscribed** from Grammar of the Week.',
            'description': '''This channel will no longer receive the Grammar of the Week.
            To subscribe again, send `/subscribe`.''',
            'color': discord.Color.blue()
        },
        'already': {
            'private': {
                ...
                'description': '''You are already unsubscribed from the Grammar of the Week.
                To subscribe again, send `/subscribe`.''',
                'color': discord.Color.dark_purple()
            },
            'guild': {
                'title': 'This channel is already unsubscribed.',
                'description': '''This channel is already unsubscribed from the Grammar of the Week.
                To subscribe again, send `/subscribe`.''',
                'color': discord.Color.dark_purple()
            }
        }
...
async def on_ready():
    ...
    client.loop.create_task(send_wotd_loop())  # Start the Grammar of the Week loop
    except Exception as e:
        log_exception(f'Error in on_ready: {str(e)}')

@client.tree.command(name='subscribe', description='Subscribe to the Grammar of the Week.')
...
@client.tree.command(name='unsubscribe', description='Unsubscribe from the Grammar of the Week.')
...
@app_commands.describe(dmy='Whether to use DD-MM-YYYY format (default is True).',
                       time='The time to send the Grammar of the Week (UTC).')
    ...
    try:
        if interaction.channel.type == discord.ChannelType.private:  # If the command was sent in a DM
            if subscribers.query_subscribed(interaction.user.id, None, None):
                ...
                embed = create_embed('Error', 'You are not subscribed to the Grammar of the Week.', discord.Color.red())
                await interaction.response.send_message(embed=embed, ephemeral=True)
        elif interaction.channel.type == discord.ChannelType.text or interaction.channel.type == discord.ChannelType.news:  # If the command was sent in a text channel
            if interaction.user.guild_permissions.manage_channels:
                if subscribers.query_subscribed(None, interaction.guild.id, interaction.channel.id):
                    ...
                    embed = create_embed('Error', 'This channel is not subscribed to the Grammar of the Week.', discord.Color.red())
                    await interaction.response.send_message(embed=embed, ephemeral=True)
            else:
...
@client.tree.command(name='query', description='Query the Grammar of the Week.')
async def query(interaction: discord.Interaction):
    ...
    # Get the grammar entry from wotd.py
        word = wotd.word
        ipa = wotd.ipa
        pos = wotd.pos
        ...
        # Get ISO week number
        week_num = datetime.datetime.strptime(current_date, '%d-%m-%Y').isocalendar()[1]
        
        if word:
            message = f'Today is {day} {month_name} {year} (Week {week_num}), and the Grammar of the Week is "{word}" ({ipa}), which is defined as: ({pos}) {definition}'
        else:
            message = f'Today is {day} {month_name} {year} (Week {week_num}), and there is no Grammar of the Week this week.'
...
@client.tree.command(name='query_date', description='Query the Grammar of the Week for a specific date.')
@app_commands.describe(date='The date to query the Grammar of the Week for (DD-MM-YYYY format).')
async def query_date(interaction: discord.Interaction, date: str):
    try:
        ...
        if datetime.datetime.now(tz.tzutc()) < datetime.datetime(int(year), int(month), int(day), tzinfo=tz.tzutc()):
            ...
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
    # Get the Grammar of the Week for the specified date
    result = wotd.query_word(date)
        if result:
            ...
            # Get ISO week number for the queried date
            query_date = datetime.datetime.strptime(date, '%d-%m-%Y')
            week_num = query_date.isocalendar()[1]
            
            # Check if the query date is the current date
            if date == wotd.current_date:
                message = f'The Grammar of the Week for today ({date_message}, Week {week_num}) is "{result['word']}" ({result['ipa']}), which is defined as: ({result['pos']}) {result['definition']}'
            else:
                message = f'The Grammar of the Week for {date_message} (Week {week_num}) was "{result['word']}" ({result['ipa']}), which is defined as: ({result['pos']}) {result['definition']}'
            if result['word'] == '':
                message = f'The Grammar of the Week for {date_message} is not available.'
            await interaction.response.send_message(message, ephemeral=True)
        else:
            embed = create_embed('Error', f'No Grammar of the Week found for {date}.', discord.Color.red())
            await interaction.response.send_message(embed=embed, ephemeral=True)
    except Exception as e:
...
@client.tree.command(name='find_wotd', description='Find the Grammar of the Week by term.')
@app_commands.describe(word='The term to find the Grammar of the Week for.')
async def find_wotd(interaction: discord.Interaction, word: str):
    try:
        result = wotd.find_wotd(word)  # May send multiple results
    # Send only what date(s) the Grammar of the Week was found
        if result:
            if len(result) == 1:
                message = f'The Grammar of the Week "{word}" was found on the following date:\n'
            else:
                message = f'The Grammar of the Week "{word}" was found on the following dates:\n'
            for row in result:
                date = row['date']
                day = date.split('-')[0]
                day = day.lstrip('0')
            ...
            embed = create_embed('Error', f'No Grammar of the Week found for "{word}".', discord.Color.red())
            await interaction.response.send_message(embed=embed, ephemeral=True)
    except Exception as e:
...
async def append(ctx, *, args: str):
    ...
    await ctx.send('Invalid arguments. Please provide the grammar term, IPA, type, and definition surrounded by quotes.')
        return
    ...
    if result:
    await ctx.send(f'The term "{word}" already exists in the database. Please use a different term.')
        return
    ...
    await ctx.send(f'''
The grammar term "{word}" has been successfully appended to the database:
- Date: {date}
- Word: {word}
- IPA: {ipa}
- PoS: {pos}
...
async def query_next(ctx):
    ...
    next_monday = date + timedelta(days=(7 - date.weekday()) or 7)  # Next Monday or the Monday after if already Monday
    date_next = next_monday.strftime('%d-%m-%Y')
    wotd_next = wotd.query_word(date_next)

    day = date_next.split('-')[0]
    day = day.lstrip('0')
    month_name = datetime.datetime.strptime(date_next, '%d-%m-%Y').strftime('%B')
    year = date_next.split('-')[2]
    date_next_formatted = f'{day} {month_name} {year}'

    message = f'Next Monday is {date_next_formatted}, and the Grammar of the Week will be "{wotd_next['word']}" ({wotd_next['ipa']}), which is defined as: ({wotd_next['pos']}) {wotd_next['definition']}'
    if wotd.word == '':
    message = f'The Grammar of the Week for tomorrow ({date_tomorrow}) is not available.'
...
async def set_wotd(ctx, *, args: str):
    ...
    matches = re.findall(pattern, args)
    if len(matches) != 5:
    await ctx.send('Invalid arguments. Please provide the date, term, IPA, type, and definition surrounded by quotes.')
        return
    lexer = shlex.shlex(args, posix=True)
    lexer.whitespace_split = True
    lexer.quotes = '"\''
    ...
    # Inform the user that the Grammar of the Week was successfully set
    await ctx.send(f'''
The Grammar of the Week has been successfully set to:
- Date: {date}
- Word: {word}
- IPA: {ipa}
- PoS: {pos}
...
async def test_send(ctx):  # This command is for testing the custom message format system
    ...
    if ctx.channel.type == discord.ChannelType.private:
        ...
        return
    if not subscriber:
    await ctx.send('You are not subscribed to the Grammar of the Week.')
        return
    try:
        # wotd_data = wotd.query_word(wotd.current_date)
        wotd_data = wotd.query_word('01-01-2025')
        if not wotd_data:
            await ctx.send('The Grammar of the Week is not available.')
            return
    ...
    message = f'Today is {current_date}, and the Grammar of the Week is "{wotd_data['word']}" ({wotd_data['ipa']}), which is defined as: ({wotd_data['pos']}) {wotd_data['definition']}'
        await ctx.send(message)
    except KeyError:
...
async def send_wotd_loop():
    ...
    log_info('Grammar of the Week Discord Bot loop started')
    ...
    while True:
        try:
            ...
            # Get the Grammar of the Week for the current date
            wotd_data = wotd.query_word(current_date_str)
            if not wotd_data:
                log_warning(f'No Grammar of the Week found for {current_date_str}. Skipping this hour.')
                continue

            # Format the Grammar of the Week message
            ...
            message_dmy = f'Today is {day} {month_name} {year}, and the Grammar of the Week is "{wotd_data["word"]}" ({wotd_data["ipa"]}), which is defined as: ({wotd_data["pos"]}) {wotd_data["definition"]}'
            message_mdy = f'Today is {month_name} {day}{day_suffix}, {year}, and the Grammar of the Week is "{wotd_data["word"]}" ({wotd_data["ipa"]}), which is defined as: ({wotd_data["pos"]}) {wotd_data["definition"]}'

            # Find the subscribers who want to receive the Grammar of the Week at this hour
